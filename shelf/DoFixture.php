...
require_once('PHPFIT_TypeAdapter_PhpTolerant.php'); //in shelf folder
require_once('DoCells.php'); //in shelf folder
...
class DoFixture extends PHPFIT_Fixture {
	...
	protected function interpretTablesInit() {
		//shelf_ClassHelper::adapterType('PhpTolerant'); not necessary, is default
	}
...
//print get_class($this). " no method: '$name' on: "; print_r($sut);
	...
	function getArgTypesForMethod($classOrObject, $name, $params) {
		return shelf_ClassHelper::getArgTypesForMethod($classOrObject, $name);
	} 
	
	function columnSetUp($targetClassName) {
		$fixture = $this->loadFixture('shelf.ColumnSetUp');
